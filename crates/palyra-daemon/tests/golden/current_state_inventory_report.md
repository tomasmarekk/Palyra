...
- CLI families: `60`
- Method registry entries: `508`
- Compat routes registered: `16/16`
- Feature rollout flags: `35`
- Runtime preview controls: `8` capabilities
- Feature rollout maturity: `scaffold=1`, `preview_only=25`, `gated_production=4`, `stable=0`, `deprecated=0`, `blocked=5`
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `acp_runtime` | `false` | `default` | `preview_only` | `acp runtime` | operator diagnostics: /console/v1/diagnostics, /admin/v1/status, palyra doctor --json | `feature_rollouts.acp_runtime` | `PALYRA_EXPERIMENTAL_ACP_RUNTIME` | `Enable feature_rollouts.acp_runtime or PALYRA_EXPERIMENTAL_ACP_RUNTIME after required tests and owner acceptance pass.`<br>`ACP actors must route permission, session, replay, and compaction handoff through host-owned control-plane boundaries` |
| `advisor_fanout` | `false` | `default` | `preview_only` | `advisors` | operator diagnostics: /console/v1/diagnostics, /admin/v1/status, palyra doctor --json | `feature_rollouts.advisor_fanout` | `PALYRA_EXPERIMENTAL_ADVISOR_FANOUT` | `Enable feature_rollouts.advisor_fanout or PALYRA_EXPERIMENTAL_ADVISOR_FANOUT after required tests and owner acceptance pass.`<br>`advisor fanout must enforce budget governance, trace attribution, and non-authoritative finalization` |
| `agent_harness_runtime` | `false` | `default` | `preview_only` | `application/agent_harness` | operator diagnostics: /console/v1/diagnostics, /admin/v1/status, palyra doctor --json | `feature_rollouts.agent_harness_runtime` | `PALYRA_EXPERIMENTAL_AGENT_HARNESS_RUNTIME` | `Enable feature_rollouts.agent_harness_runtime or PALYRA_EXPERIMENTAL_AGENT_HARNESS_RUNTIME after required tests and owner acceptance pass.`<br>`harness selection, callbacks, transcript mirroring, and lifecycle attempts must stay host-owned before production routing` |
...
| `execution_backend_docker` | `false` | `default` | `blocked` | `execution backends` | internal runtime flag; externally visible only through diagnostics and doctor | `feature_rollouts.execution_backend_docker` | `PALYRA_EXPERIMENTAL_EXECUTION_BACKEND_DOCKER` | `Enable feature_rollouts.execution_backend_docker or PALYRA_EXPERIMENTAL_EXECUTION_BACKEND_DOCKER after required tests and owner acceptance pass.`<br>`real Docker execution still needs operator-enabled runtime coverage before production promotion` |
...
| `execution_backend_ssh_tunnel` | `false` | `default` | `blocked` | `execution backends` | internal runtime flag; externally visible only through diagnostics and doctor | `feature_rollouts.execution_backend_ssh_tunnel` | `PALYRA_EXPERIMENTAL_EXECUTION_BACKEND_SSH_TUNNEL` | `Enable feature_rollouts.execution_backend_ssh_tunnel or PALYRA_EXPERIMENTAL_EXECUTION_BACKEND_SSH_TUNNEL after required tests and owner acceptance pass.`<br>`SSH worker RPC transport trust chain still needs live tunnel coverage before production promotion` |
...
| `inline_runtime_hooks` | `false` | `default` | `preview_only` | `hooks/runtime loop` | operator diagnostics: /console/v1/diagnostics, /admin/v1/status, palyra doctor --json | `feature_rollouts.inline_runtime_hooks` | `PALYRA_EXPERIMENTAL_INLINE_RUNTIME_HOOKS` | `Enable feature_rollouts.inline_runtime_hooks or PALYRA_EXPERIMENTAL_INLINE_RUNTIME_HOOKS after required tests and owner acceptance pass.`<br>`inline hook invocation must preserve timeout, panic, policy, approval, and audit fail-closed behavior` |
| `lsp_service` | `false` | `default` | `preview_only` | `code intelligence` | operator diagnostics: /console/v1/diagnostics, /admin/v1/status, palyra doctor --json | `feature_rollouts.lsp_service` | `PALYRA_EXPERIMENTAL_LSP_SERVICE` | `Enable feature_rollouts.lsp_service or PALYRA_EXPERIMENTAL_LSP_SERVICE after required tests and owner acceptance pass.`<br>`LSP lifecycle must keep diagnostics bounded, workspace-scoped, and restart-safe before model-visible tools consume it` |
...
| `provider_recovery` | `false` | `default` | `preview_only` | `model provider recovery` | internal runtime flag; externally visible only through diagnostics and doctor | `feature_rollouts.provider_recovery` | `PALYRA_EXPERIMENTAL_PROVIDER_RECOVERY` | `Enable feature_rollouts.provider_recovery or PALYRA_EXPERIMENTAL_PROVIDER_RECOVERY after required tests and owner acceptance pass.`<br>`recovery classifiers must keep retries bounded, idempotent, and visible through replay-safe reason codes` |
| `provider_stream_normalizer` | `false` | `default` | `preview_only` | `model provider streaming` | internal runtime flag; externally visible only through diagnostics and doctor | `feature_rollouts.provider_stream_normalizer` | `PALYRA_EXPERIMENTAL_PROVIDER_STREAM_NORMALIZER` | `Enable feature_rollouts.provider_stream_normalizer or PALYRA_EXPERIMENTAL_PROVIDER_STREAM_NORMALIZER after required tests and owner acceptance pass.`<br>`stream normalization must keep malformed chunks, idle timeouts, duplicate deltas, late usage, and public failed SSE frames covered` |
...
| `terminal_sessions` | `false` | `default` | `preview_only` | `process runtime` | operator diagnostics: /console/v1/diagnostics, /admin/v1/status, palyra doctor --json | `feature_rollouts.terminal_sessions` | `PALYRA_EXPERIMENTAL_TERMINAL_SESSIONS` | `Enable feature_rollouts.terminal_sessions or PALYRA_EXPERIMENTAL_TERMINAL_SESSIONS after required tests and owner acceptance pass.`<br>`persistent terminal sessions must keep process handles, cwd, env, sudo posture, and cleanup evidence bounded` |
