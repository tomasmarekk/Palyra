...
pub(crate) const INSTRUCTION_COMPILER_VERSION: u32 = 36;
...
fn tool_specific_contract(tool_names: &[String]) -> String {
    ...
    if tool_names.iter().any(|tool| tool == "palyra.process.run") {
        ...
        contracts.push("palyra.process.run background lifecycle addendum: the current local executor exposes run-owned processes only. Keep temporary servers alive only while the agent run is active, stop them explicitly when practical, and state that they are not available after terminal run cleanup. Durable detached or hidden-verifier process handoff is unavailable until the executor can persist and enforce that lifecycle; do not request compatibility-only detached fields.".to_owned());
    }
...
mod tests {
    ...
    fn tool_specific_contract_explains_process_runner_limits() {
        ...
        assert!(contract.contains("background lifecycle addendum"));
        assert!(contract.contains("exposes run-owned processes only"));
        assert!(contract.contains("process handoff is unavailable"));
        assert!(contract.contains("do not request compatibility-only detached fields"));
    ...
    fn tool_specific_contract_explains_browser_visible_text_evidence() {
        ...
        assert!(contract.contains("file:///workspace/<workspace-relative-path>"));
    ...
    fn tool_specific_contract_explains_cross_session_memory() {
        ...
        assert!(contract.contains("follow the effective tool posture"));
        assert!(contract.contains("do not ask for approval"));
