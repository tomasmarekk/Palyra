//! Application layer of the daemon: channel routing, session orchestration,
//! tool execution, memory, and approval flows built on top of the gateway
//! runtime and journal. Submodules are grouped by capability; `route_message`
//! and `run_stream` are the inbound and provider-turn entry points.

pub mod advisor_fanout;
pub mod agent_harness;
pub mod agent_harness_callbacks;
pub mod agent_harness_context;
pub mod agent_harness_host;
pub mod agent_harness_lifecycle;
pub(crate) mod agent_harness_provider_bridge;
pub mod agent_harness_threads;
pub mod agent_harness_tool_bridge;
pub mod agent_harness_v2;
pub mod approvals;
pub mod auth;
pub mod channel_commands;
pub mod channel_turn;
pub mod channels;
pub mod clarify;
pub mod code_intel_runtime;
pub mod codex_app_server_bridge;
pub mod codex_app_server_harness;
pub mod coding_runtime;
pub mod context_compaction;
pub mod context_compression;
pub mod context_engine;
pub mod context_lifecycle;
pub mod context_recovery;
pub mod context_references;
pub mod conversation_bindings;
pub mod core_stability;
pub(crate) mod daemon_lifecycle;
pub mod delivery_arbitration;
pub mod execution_gate;
pub mod external_agent_harness;
pub mod file_view_registry;
pub mod inbound_coalescer;
pub mod instruction_compiler;
pub mod learning;
pub mod local_resource_governor;
pub mod lsp_document_sync;
pub mod lsp_workspace_supervisor;
pub mod managed_coding_diagnostics;
pub mod managed_coding_recovery;
pub mod managed_coding_services;
pub mod managed_runtime;
pub mod managed_worktree_executor;
pub mod managed_worktree_snapshots;
#[cfg(test)]
pub(crate) mod managed_worktree_test_support;
pub mod managed_worktrees;
pub mod mcp_broker;
pub mod mcp_runtime;
pub mod memory;
pub mod memory_provider;
pub mod multimodal_context;
pub mod native_compaction_handoff;
pub mod native_harness_relay;
pub(crate) mod objective_continuation;
pub(crate) mod objective_guards;
pub mod outbound_lifecycle;
pub mod plan_state;
pub mod process_supervisor;
pub mod progress_draft;
pub mod project_context;
pub mod project_context_summary;
pub mod project_facts;
pub mod provider_events;
pub mod provider_input;
pub mod provider_turn_recovery;
pub mod pty_backend;
pub mod recall;
pub mod release_hardening;
pub mod replay_continuity;
pub(crate) mod restart_coordinator;
pub mod resume_classifier;
pub(crate) mod run_admission;
pub mod runtime_boundary_metrics;
pub(crate) mod runtime_kernel_v2;
pub mod runtime_resource_manifest;
pub mod service_authorization;
pub mod session_compaction;
pub mod session_pruning;
pub mod session_queue;
pub mod side_effect_reconciliation;
pub mod terminal_sessions;
pub mod tool_governance;
pub mod tool_jobs;
pub mod tool_registry;
pub mod tool_runtime;
pub mod tool_security;
pub mod turn_control;
pub mod verification;
pub(crate) mod wake_coordinator;
pub(crate) mod work_graph_coordinator;
pub mod workspace_observability;

pub mod route_message;
pub mod run_stream;
