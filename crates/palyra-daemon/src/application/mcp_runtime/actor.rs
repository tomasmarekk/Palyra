...
use super::{
    supervisor::{
        ...
        McpRuntimeStoreError, McpRuntimeSupervisorError, McpServerRecordV2,
    },
    transport::{
        ...
        McpTransportError, McpTransportEvent,
    },
};
...
impl McpActorCore {
    ...
    async fn normalize_restored_state(&mut self) -> Result<(), McpActorError> {
        let next = match self.record.lifecycle {
            McpRuntimeLifecycleState::Ready | McpRuntimeLifecycleState::Handshaking => {
            ...
            tokio::select! {
                ...
                event = reader.next_event() => {
                    ...
                    if let Some(reason_code) =
                        self.handle_transport_event(event, &mut writer, &mut pending).await?
                    {
                        if let Some(drain) = drain.as_mut() {
                            drain.deadline = Instant::now();
                            continue;
    ...
    async fn record_transport_failure(&mut self, reason_code: &str) -> Result<(), McpActorError> {
        let next =
            self.record.mark_failure(&self.config.reconnect_policy, reason_code, now_unix_ms())?;
        self.commit(next, reason_code).await
    }
...
mod tests {
    ...
    use crate::application::mcp_runtime::{
        ...
        McpResponsePayload, McpSessionTransportKind,
    };
    ...
    impl McpSessionConnector for FakeConnector {
        async fn connect(
            ...
            self.session.lock().expect("connector mutex is healthy").take().ok_or_else(|| {
                McpTransportError::Unavailable {
    ...
    fn spawn_harness() -> Harness {
        ...
        let parts = McpSessionActor::spawn(
            actor_config(record),
            connector,
            store,
            Arc::new(RejectingCallbacks),
        )
        .expect("actor starts");
