...
use crate::{
    ...
    application::{
        channel_turn::{ChannelBotLoopGuard, ChannelHistoryStore},
    ...
    journal::{
        ...
        ApprovalRiskLevel, ApprovalSubjectType, ApprovalsListFilter, AutonomousWakeAdmissionRecord,
        AutonomousWakeAdmissionRequest, CanvasStatePatchRecord, CanvasStateSnapshotRecord,
        CanvasStateTransitionRequest, CronConcurrencyPolicy, CronJobCreateRequest, CronJobRecord,
        CronJobUpdatePatch, CronJobsListFilter, CronRunFinalizeRequest, CronRunRecord,
        CronRunStartRequest, CronRunStatus, CronRunsListFilter, JournalAppendRequest, JournalError,
        JournalEventRecord, JournalStore, MemoryEmbeddingsBackfillOutcome, MemoryItemCreateRequest,
        ...
        OrchestratorRunStatusSnapshot, OrchestratorRunTerminalSettlementRequest,
...
pub(crate) const RUN_PARAMETER_DELTA_CACHE_MAX_ENTRY_BYTES: usize = 64 * 1024;
pub(crate) const RUN_PARAMETER_DELTA_CACHE_MAX_TOTAL_BYTES: usize = 8 * 1024 * 1024;
...
pub(crate) const MAX_CANVAS_IN_MEMORY_RECORDS: usize = 128;
pub(crate) const MAX_CANVAS_RECOVERY_SNAPSHOTS: usize = MAX_CANVAS_IN_MEMORY_RECORDS;
...
pub(crate) const VAULT_METADATA_TOOL_NAME: &str = "palyra.vault.metadata";
...
pub(crate) const BROWSER_VISION_TOOL_NAME: &str = "palyra.browser.vision";
pub(crate) const BROWSER_IMAGES_LIST_TOOL_NAME: &str = "palyra.browser.images.list";
pub(crate) const BROWSER_DIALOG_TOOL_NAME: &str = "palyra.browser.dialog";
pub(crate) const BROWSER_CDP_INVOKE_TOOL_NAME: &str = "palyra.browser.cdp.invoke";
...
pub(crate) async fn execute_tool_with_runtime_dispatch_with_cancellation_and_progress(
        ...
        local_tool_runtime_runner_unavailable_outcome(context, proposal_id, tool_name, input_json)
    {
        ...
        .await
    } else if tool_name == VAULT_METADATA_TOOL_NAME {
        crate::application::tool_runtime::vault::execute_vault_metadata_tool(
            runtime_state,
            context,
            proposal_id,
            input_json,
        ...
        tool_name,
        DELEGATION_QUERY_TOOL_NAME
            | DELEGATION_CONTROL_TOOL_NAME
            | SESSIONS_SPAWN_TOOL_NAME
        ...
        outcome
    } else if tool_name == PROCESS_SEND_KEYS_TOOL_NAME {
        let config =
            process_runner_tool_config_for_session(runtime_state, context, input_json).await;
        if !config.process_runner.enabled {
            return build_tool_execution_outcome(
        ...
        let outcome = execute_process_send_keys_tool(
            &config,
            proposal_id,
            PROCESS_SEND_KEYS_TOOL_NAME,
            input_json,
        ...
        .await;
        if outcome.success {
            record_process_keys_journal_event(runtime_state, context, proposal_id, input_json)
                .await;
        }
        outcome
        ...
        Box::pin(execute_persistent_mcp_tool(
            runtime_state,
            context,
            proposal_id,
            tool_name,
...
async fn execute_process_send_keys_tool(
    config: &ToolCallConfig,
    proposal_id: &str,
    tool_name: &str,
    input_json: &[u8],
    ...
    let result = tokio::time::timeout(
        Duration::from_millis(PROCESS_INPUT_WRITE_TIMEOUT_MS),
        tokio::task::spawn_blocking(move || {
            crate::sandbox_runner::send_keys_to_background_process(input.as_slice())
        }),
    )
    .await;
    match result {
        ...
        Ok(Err(_)) => build_tool_execution_outcome(
            ...
            false,
            b"{}".to_vec(),
            "palyra.process.send_keys worker panicked".to_owned(),
            false,
            crate::sandbox_runner::process_runner_executor_name(&config.process_runner),
            crate::sandbox_runner::process_runner_sandbox_enforcement_label(&config.process_runner),
        ),
        Err(_) => build_tool_execution_outcome(
            ...
            false,
            b"{}".to_vec(),
            format!("palyra.process.send_keys timed out after {PROCESS_INPUT_WRITE_TIMEOUT_MS}ms"),
            true,
            crate::sandbox_runner::process_runner_executor_name(&config.process_runner),
            crate::sandbox_runner::process_runner_sandbox_enforcement_label(&config.process_runner),
        ),
...
async fn record_process_keys_journal_event(
    runtime_state: &Arc<GatewayRuntimeState>,
    context: ToolRuntimeExecutionContext<'_>,
    proposal_id: &str,
    input_json: &[u8],
    ...
    let payload = json!({
        "schema_version": 1,
        "event_type": "process.keys.sent",
        "tool_name": PROCESS_SEND_KEYS_TOOL_NAME,
        "proposal_id": proposal_id,
        "session_id": context.session_id,
        "run_id": context.run_id,
        "pid": pid,
        "keys": "<redacted>",
        "keys_sha256": sha256_hex(input_json),
        "created_at_unix_ms": current_unix_ms(),
        "redaction_level": "input_redacted",
    });
    let payload_json = match serde_json::to_vec(&payload) {
        ...
        Err(error) => {
            warn!(
                proposal_id,
                error = %error,
                "failed to serialize process keys journal payload"
            );
            return;
        }
    };
        ...
        .await
    {
        warn!(
            ...
            status_code = ?error.code(),
            status_message = %error.message(),
            "process keys journal write failed"
        );
    }
}
...
async fn process_runner_workspace_roots_for_session(
    ...
    let configured_root = config.process_runner.workspace_root.clone();
    let fallback = vec![configured_root.clone()];
