;; Reference run-lifecycle hook guest for executable ABI v2 conformance.
;; The host validates the returned action against the binding-approved hook
;; role, so this guest cannot promote itself to a stronger role.
(module
  ...
  (func (export "palyra-abi-v2-invoke")
    ...
    local.get $request-pointer i32.load8_u offset=10 i32.const 4 i32.ne
    if i32.const -10 return end
