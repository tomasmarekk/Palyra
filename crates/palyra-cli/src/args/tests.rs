...
fn parse_mcp_add_registry_server() {
    let parsed = Cli::parse_from([
        ...
        "--sampling-mode",
        "allowlist",
        "--sampling-model-capability",
        "gpt-5",
        "--sampling-host-model-id",
        "gpt-5",
        "--sampling-max-output-tokens-per-request",
        "256",
        "--sampling-window-seconds",
        "60",
        "--sampling-max-requests-per-window",
        "4",
        "--sampling-max-output-tokens-per-window",
        "1024",
        "--tool-allow",
        "search",
        "--enabled",
        ...
        Command::Mcp {
            command: McpCommand {
                subcommand: McpSubcommand::Add(McpRegistryMutateArgs {
                    ...
                    sampling_mode: McpSamplingModeArg::Allowlist,
                    sampling_model_capabilities: vec!["gpt-5".to_owned()],
                    sampling_host_model_id: Some("gpt-5".to_owned()),
                    sampling_max_output_tokens_per_request: 256,
                    sampling_window_seconds: 60,
                    sampling_max_requests_per_window: 4,
                    sampling_max_output_tokens_per_window: 1_024,
    ...
    let cases: &[(&[&str], &[&str])] = &[
        ...
        (
            ...
            &[
                ...
                "Expose tools classified as sensitive",
                "use --approval-mode to control approvals",
