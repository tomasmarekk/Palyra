...
impl App {
    ...
    async fn handle_slash_command(&mut self, command: &str) -> Result<()> {
        ...
        match name {
            ...
            "rollback" => {
                let arguments = parts.map(ToOwned::to_owned).collect::<Vec<_>>();
                self.handle_rollback_command(arguments).await?;
            }
            "background" => {
                let arguments = parts.map(ToOwned::to_owned).collect::<Vec<_>>();
    ...
    async fn handle_rollback_command(&mut self, arguments: Vec<String>) -> Result<()> {
        let context = self.connect_admin_console().await?;
        let Some(first) = arguments.first().map(String::as_str) else {
            ...
            self.push_entry(EntryKind::System, "Workspace rollback", lines.join("\n"));
            self.status_line = "Workspace rollback checkpoints refreshed".to_owned();
            return Ok(());
        };

        if first.eq_ignore_ascii_case("diff") {
            let Some(target) = arguments.get(1).map(String::as_str) else {
                self.status_line = "Usage: /rollback diff <checkpoint_id|run_id>".to_owned();
                self.ux_metrics.record(TuiUxMetricKey::Errors);
                return Ok(());
            };
            let left_run_id = self
                .last_run_id
                .clone()
            ...
            self.push_entry(EntryKind::System, "Workspace rollback diff", lines.join("\n"));
            self.status_line = "Workspace rollback diff loaded".to_owned();
            return Ok(());
        }
            ...
            .await
        {
            Ok(payload) => {
                ...
                self.status_line = "Workspace checkpoint detail loaded".to_owned();
            }
            Err(_) => {
                let payload = context
                    .client
                ...
                self.status_line = "Workspace rollback run detail loaded".to_owned();
            }
        }
        Ok(())
    }
