...
export function AuthSection({ app }: AuthSectionProps) {
  ...
  const profiles = [...app.authProfiles].sort((left, right) =>
  ...
  const healthById = new Map((app.authHealth?.profiles ?? []).map((profile) => [profile.profile_id, profile]));
  ...
  return (
    <main className="workspace-page">
      <WorkspacePageHeader
        ...
        status={
          <>
            <WorkspaceStatusChip tone={workspaceToneForState(app.authProviderState?.state ?? "unknown")}>
      ...
      <section className="workspace-metric-grid workspace-metric-grid--compact">
        <WorkspaceMetricCard label="Default profile" value={app.authProviderState?.default_profile_id ?? "none"} detail="Explicit default selection remains visible." tone={app.authProviderState?.default_profile_id ? "success" : "default"} />
        <WorkspaceMetricCard label="Healthy" value={summary.ok + summary.staticCount} detail={`${summary.expiring} expiring, ${summary.expired} expired, ${summary.missing} missing`} tone={summary.expired > 0 || summary.missing > 0 ? "danger" : summary.expiring > 0 ? "warning" : "success"} />
        <WorkspaceMetricCard label="OAuth bootstrap" value={app.authProviderState?.bootstrap_supported ? "Ready" : "Unavailable"} detail={app.authProviderState?.note ?? "Provider contract posture."} tone={app.authProviderState?.bootstrap_supported ? "success" : "default"} />
      </section>

      <section className="workspace-aside-grid">
        <div className="workspace-stack">
          <WorkspaceSectionCard title="Profile inventory" description="Readable list first, with scope and health visible before any edit flow.">
            {profiles.length === 0 ? (
              <WorkspaceEmptyState title="No profiles configured" description="Create an API-key or OAuth profile to make provider auth available to chat and agent flows." />
            ) : (
              <WorkspaceTable ariaLabel="OpenAI profiles" columns={["Profile", "Scope", "Credential", "State", "Actions"]}>
                {profiles.map((profile) => {
                  const health = healthById.get(profile.profile_id);
                  const state = health?.state ?? (profile.credential.type === "oauth" ? "managed" : "static");
                  const scopeLabel = profile.scope.kind === "agent" ? `agent:${profile.scope.agent_id ?? "unassigned"}` : "global";
                  return (
                    <tr key={profile.profile_id}>
                      <td><div className="workspace-table__meta"><strong>{profile.profile_name}</strong><span className="chat-muted">{profile.profile_id}</span></div></td>
                      <td>{scopeLabel}</td>
        ...
        <div className="workspace-stack">
          <WorkspaceSectionCard title="Selected profile" description="Inspect scope, vault references, expiry, and safe recovery actions without opening raw config.">
            {selectedProfile === null ? (
              <WorkspaceEmptyState title="Nothing selected" description="Choose a profile from the inventory to inspect details and available actions." compact />
            ) : (
              <SelectedProfileCard app={app} profile={selectedProfile} health={healthById.get(selectedProfile.profile_id)} />
            )}
          </WorkspaceSectionCard>
    ...
    <div className="workspace-stack">
      <div className="workspace-inline">
        ...
        <WorkspaceStatusChip tone={profile.scope.kind === "agent" ? "accent" : "default"}>
          {profile.scope.kind === "agent" ? `agent:${profile.scope.agent_id ?? "unassigned"}` : "global"}
        </WorkspaceStatusChip>
      ...
      <dl className="workspace-key-value-grid">
        ...
        <div><dt>Updated</dt><dd>{formatUnixMs(profile.updated_at_unix_ms)}</dd></div>
        <div><dt>Created</dt><dd>{formatUnixMs(profile.created_at_unix_ms)}</dd></div>
        <div><dt>Expiry</dt><dd>{formatUnixMs(health?.expires_at_unix_ms ?? (profile.credential.type === "oauth" ? profile.credential.expires_at_unix_ms : undefined))}</dd></div>
      </dl>
      ...
      {profile.credential.type === "api_key" ? (
        ...
        <div className="workspace-stack">
          <WorkspaceRedactedValue label="Access token vault ref" value={oauthCredential?.access_token_vault_ref} sensitive revealed={app.revealSensitiveValues} allowCopy />
          <WorkspaceRedactedValue label="Refresh token vault ref" value={oauthCredential?.refresh_token_vault_ref} sensitive revealed={app.revealSensitiveValues} allowCopy />
          <WorkspaceRedactedValue label="Client secret vault ref" value={oauthCredential?.client_secret_vault_ref} sensitive revealed={app.revealSensitiveValues} allowCopy placeholder="No client secret vault ref published" />
          <dl className="workspace-key-value-grid">
            <div><dt>Token endpoint</dt><dd>{oauthCredential?.token_endpoint ?? "n/a"}</dd></div>
            <div><dt>Scopes</dt><dd>{oauthCredential?.scopes.join(", ") || "n/a"}</dd></div>
            <div><dt>Refresh failures</dt><dd>{readNumber((oauthCredential?.refresh_state ?? {}) as JsonObject, "failure_count") ?? 0}</dd></div>
            <div><dt>Last success</dt><dd>{formatUnixMs(readNumber((oauthCredential?.refresh_state ?? {}) as JsonObject, "last_success_unix_ms"))}</dd></div>
          </dl>
        </div>
      )}
